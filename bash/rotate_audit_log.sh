...
rename_and_compress_old_logs() {
    for file in $(find /var/log/audit/ -regextype posix-egrep -regex '.*/(audit.log.[0-9]+){1,3}'); do
...
delete_old_compressed_logs() {
    ...
    rm -v $(find /var/log/audit/ -mtime 15 -regextype posix-extended -regex '.*audit\.log\..*(xz|gz|bz2)$' | sort -n | head -n -${KEEP})
}
