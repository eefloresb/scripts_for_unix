...
__STOP_SOAP_8090(){
   nkill=$(ps -fea | grep -Ei "(digitall-soap)" | grep -v grep)
   kill -s 9 $nkill
   echo "stoped ... $nkill SOAP process"
}
...
__HELP(){
    ...
    echo -e "\t$RED start the digitall-soap application$NC: ./$0 -s"  
    echo ""
    echo -e "\t$RED down the digitall-soap application$NC:   ./$0 -d"
    echo ""
    echo -e "\t$RED stop and start the digitall-soap application$NC: ./$0 -r"  
    echo ""
  exit 0 
}
...
while getopts "s:d:r" OPTS; do 
  case "$OPTS" in
      ...
      -h) __HELP;;
       *)
          echo "Ingrese valores permitidos $0 -h"
  esac  
done 
